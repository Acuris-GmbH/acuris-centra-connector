...
import type { ValidationResult, AddressInput } from "@acuris-geo/av-sdk";
...
export interface UseAcurisValidationReturn {
  ...
  error?: Error;
  validate: (address: AddressInput) => Promise<ValidationResult | undefined>;
  /** Reset to idle. */
  reset: () => void;
}

/**
 * Headless validation hook. Use this when you want to roll your own UI.
 *
 * Cancels any inflight request when `validate` is called again, so the latest
 * call always wins (avoids race conditions on fast-typing users).
 */
