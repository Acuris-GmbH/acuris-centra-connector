...
Thanks for your interest in improving the Acuris Centra connector. This repo holds two
npm packages and a sample Next.js storefront; contributions to any of them are welcome.
...
- Phase 1 is **beta**. Behaviour and types may still change between minor versions until
  we tag `1.0.0`.
- The SDK targets Node 18+ and the React package targets React 18+. We use native
  `fetch` everywhere — please don't introduce `axios`, `node-fetch`, or polyfills.

## Development setup

```bash
git clone https://github.com/Acuris-GmbH/acuris-centra-connector.git
cd acuris-centra-connector
npm install
npm run build
npm test
```
...
examples/
  centra-storefront/         # Next.js demo
docs/
...
- Which package (`acuris-av-sdk` or `acuris-centra-checkout`) and version.
